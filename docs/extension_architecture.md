...
서로 다른 프레임 간의 독립적인 상태를 동기화하고 공통 액션을 제어하기 위해 백그라운드 서버 워커가 **전역 상태 조정자(State Coordinator)** 역할을 수행합니다.

### 2-1. 중앙 집중식 액션 제어
- **데이터 흐름**:
  1. 각 프레임(Top 또는 iframe)에서 단축키가 발생하면 백그라운드로 전용 메시지(`RUN_TEXT_FLOW` 등)를 전송합니다.
  2. 백그라운드: 요청을 검증(로그인 상태 등)하고, 해당 탭의 최상위 프레임(Top Frame)에 오버레이를 띄우도록 명령합니다.
- **효과**: 프레임에 상관없이 일관된 UI/UX를 보장하며, 복잡한 상태 관리를 백그라운드에서 통합 처리합니다.
