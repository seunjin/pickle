...
- pending/active → 정상 인증 범위 내 접근 허용 (단, pending은 가입 UI로 제한)
...
- pending 상태에서는 대시보드 접근 등 주요 기능 이용이 제한되며, 홈(`/`)으로 리다이렉트되어 **회원가입 권장 확인창(Confirm Dialog)**이 표시된다.
...
- **4. Data Integrity (Cascade Deletion)**
    - 사용자 삭제(`public.users` 레코드 삭제) 시, 해당 사용자의 권한(`workspace_members`) 및 **개인 데이터(`notes`, `tags`, `folders`, `assets`)**는 **Cascade** 설정에 의해 자동 삭제된다.
